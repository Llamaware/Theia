...
# Regular expression to match bytecode entries
pattern = re.compile(r'V\((\w+),')

# Extract bytecode names using regex
bytecodes = pattern.findall(bytecode_list)

# Print bytecodes with their corresponding hex values

opcodes = []

for index, bytecode in enumerate(bytecodes):
    hex_value = f"0x{index:02X}"  # Convert index to hex with 2 digits
    print(f"{hex_value} - {bytecode}")
    opcodes.append((hex_value, bytecode))
...
with open("new_opcodes.txt", "w") as text_file:
    text_file.write(formatted_output)

print("Text file created: new_opcodes.txt")
