...
public final class ScopeInfoStore implements Serializable {
	...
	public static ScopeInfoStore fromStruct(final Object struct) {
		...
		if (cached instanceof ScopeInfoStore) {
			return (ScopeInfoStore) cached;
		}
		// Cache the object immediately before recursion
		final ScopeInfoStore placeholder = new ScopeInfoStore(null, 0, new ArrayList<>(), null);
		...
		ScopeInfoStore result = new ScopeInfoStore(((ScopeInfoStruct) struct).getName(), ((ScopeInfoStruct) struct).getAddress().getOffset(), ctxVars1, outerScope); // added
